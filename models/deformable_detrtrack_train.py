...
from .res_refiner import ResBboxRefiner
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries_one2one, num_feature_levels,
                 res_bbox_refiner: ResBboxRefiner,
                 aux_loss=True, with_box_refine=False, two_stage=False, num_queries_one2many=0, k_one2many=5):
        ...
        self.res_bbox_refiner = res_bbox_refiner
        ...
        if not two_stage:
            ...
            # NOTE: This is a trick to load pretrained weights
            self.query_embed = nn.Embedding(num_queries_one2one, hidden_dim * 2)
            self.multi_query_embed = nn.Embedding(num_queries_one2many, hidden_dim * 2)
    ...
    def forward(self, samples_targets, unused_embed=None):
        if self.training:
            ...
            srcs = features
            srcs = [nested_tensor.tensors for nested_tensor in srcs]


            # if 'pred_boxes' in pre_out and 'feat' in pre_embed:
            #     tensor_shape = samples.tensors.shape
            #     image_whwh = [tensor_shape[-2], tensor_shape[-1], tensor_shape[-2], tensor_shape[-1]]
            #     # 确保 image_whwh 是一个张量
            #     if isinstance(image_whwh, list):
            #         image_whwh = torch.tensor(image_whwh, device='cuda:0')  # 或者使用适当的设备
            #     # 处理 image_whwh 以适应 bboxes 的形状
            #     # 提取宽度和高度
            #     width, height = image_whwh[0], image_whwh[1]
            #
            #     # 扩展 image_whwh 为 (1, N, 4) 的形状
            #     image_whwh = torch.tensor([[width, height, width, height]], device='cuda:0')
            #     refined_predictions = self.res_bbox_refiner(srcs, pre_out, image_whwh)
            #     pre_embed['reference'] = torch.cat([refined_predictions['pred_boxes'], refined_predictions['pred_boxes_one2many']], dim=1)
    ...
    def forward_once(self, samples: NestedTensor, train_samples: NestedTensor, targets=None, next_targets=None):
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src = self.combine(
                        torch.cat([self.input_proj[l](features[-1].tensors), self.input_proj[l](pre_feat[-1].tensors)],
                                  dim=1))
                else:
        ...
        if not self.two_stage:
            ...
            query_embeds = torch.cat([query_embeds, self.multi_query_embed.weight], dim=0)
        # make attn mask
        ...
        self_attn_mask[self.num_queries_one2one:, :] = True
        ...
        if self.k_one2many > 0:
            ...
            if (self.num_queries - self.num_queries_one2one) % self.k_one2many > 0:
                group_num = self.k_one2many + 1
            else:
                group_num = self.k_one2many
            for group_idx in range(group_num):
                self_attn_mask[
                self.num_queries_one2one
                + group_idx * group_length: self.num_queries_one2one
                                            + (group_idx + 1) * group_length,
                self.num_queries_one2one
                + group_idx * group_length: self.num_queries_one2one
                                            + (group_idx + 1) * group_length,
                ] = False
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, memory = self.transformer(
            srcs, masks, pos, query_embeds, self_attn_mask=self_attn_mask
        )
        ...
        pre_embed = {'reference': outputs_coord, 'tgt': hs[-1], 'feat': features, 'memory': memory}
    ...
    def forward_train(self, samples: NestedTensor, pre_embed=None):
        ...
        if pre_embed is not None:
            pre_reference, pre_tgt, pre_feat, pre_memory = pre_embed['reference'], pre_embed['tgt'], pre_embed['feat'], \
            pre_embed['memory']
        else:
            pre_reference = None
            pre_tgt = None
        ...
        # 构造原始 self-attention 掩码
        # self_attn_mask = torch.zeros((self.num_queries, self.num_queries), dtype=torch.bool, device=src.device)
        # self_attn_mask[self.num_queries_one2one:, :] = True
        ...
        # # 基于参考点定义一对多匹配掩码
        # reference_points = pre_reference[:, :, :2]  # 假设参考点的 (x, y) 坐标
        # radius = 0.1  # 自定义半径，用于控制参考点的邻域范围
        # # group_length = (self.num_queries - self.num_queries_one2one) // self.k_one2many
        # group_length = (
        #                        self.num_queries - self.num_queries_one2one
        #                ) // self.k_one2many
        # if (self.num_queries - self.num_queries_one2one) % self.k_one2many > 0:
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, _ = self.transformer(
            srcs, masks, pos, query_embeds, pre_reference, pre_tgt, self_attn_mask=self_attn_mask
        )
...
def build(args):
    ...
    res_bbox_refiner = ResBboxRefiner(d_model=64, num_reg_layers=2)
    ...
    model = DeformableDETR(
        ...
        res_bbox_refiner=res_bbox_refiner
    )
    if args.masks:
